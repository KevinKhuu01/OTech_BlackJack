...
## Overview
This is our final project for the Software Systems Devolopment and Integration course, in which we were given the freedom to choose what sort of project groups would like to do. This final project for is an Ontario Tech themed BlackJack app, featuring a betting function, cards and tables with an Ontario Tech inspired design and multiplayer capabilities (up to 3 players at a table). Players can create their own account complete with a starting balance of $10,000 and can add funds whenever they would like to. After logging in, players can create their own public tables to start playing or join other tables with their friends to play against each other and the dealer. We hope you enjoy Ontario Tech Blackjack! 
...
- 2: Open your preferred terminal and navigate to the directory you want to clone the program into. You can also create a new folder to store it.
...
- 6: Next, locate the Client java file (`src\main\java\frontend`) and double click it to open. run this file with the green play button and your ready to start Ontario Tech BlackJack!
- ***Note:*** In order to use multiplayer features, you must enable multiple client instances to be run at the same time. You can enable this by navigating to **Run -> Edit Configurations -> Client -> Modify Options** and checkmark the 'Allow multiple instances' option to have multiple players on one device at a single time.
